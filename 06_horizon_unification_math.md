...
다중 행렬 모델에서 $X^a$들은 서로 교환하지 않으므로($[X_a, X_b] \neq 0$), 단순한 1차원 위그너 반원 법칙이나 스칼라 고윳값 분포는 적용될 수 없다. 대신, 대형 $N$ 극한에서 비가환 행렬들의 결합 분포(Joint distribution)는 보이쿨레스쿠(Voiculescu)의 **자유 확률론(Free Probability Theory)**에 의해 지배된다.
비가환 쿨롱 기체 역학에 의해, 이 행렬들은 유계(bounded) 작용소 대수를 형성하며 특정한 콤팩트 스펙트럼 반경을 갖는 **"퍼지 액적(Fuzzy Droplet)"**을 기하학적으로 구성한다. 다변수 자유 반원 법칙(Multivariate Free Semicircular Law)에 의해 이 액적의 대수적 스펙트럼 경계 너머에는 어떠한 미시 상태도 존재할 수 없으며, 이 엄밀한 스펙트럼 경계가 거시 우주의 절대적 지평선 $R_H$를 동역학적으로 형성한다.

**명제 4.1 (홀로그래픽 자기 일관성).** 비가환 행렬 모형의 자유 확률론은 두 가지 핵심 성질을 자연스럽게 산출한다: (i) 미시정준(microcanonical) 엔트로피가 $S_{\text{micro}} = N^2$ ($N \times N$ 행렬의 독립 자유도 수)로 스케일링하며, (ii) 비가환 작용소 대수가 날카로운 스펙트럼 경계를 가진 콤팩트한 기하학적 영역을 형성한다. 베켄슈타인-호킹 공식은 행렬 스케일과 중력 스케일 사이의 유일한 자기 일관적 동정(identification)으로서 창발한다:
...
이 팽창을 주도하는 터널링 상수 $\gamma$는 현상론적 매개변수가 아니라, 비가환 양-밀스 작용량 $\text{Tr}([X_a, X_b]^2)$의 엄밀한 **인스턴톤 해(Instanton Solution)**로부터 유도된다. 퍼지 구면(Fuzzy Sphere)을 형성하는 배경 진공 $X_a^{(0)}$ 주위의 양자 요동 $\delta X$는 $N \to N+1$ 차원으로 전이하는 비가환 인스턴톤으로 작용한다. 이 인스턴톤 배위의 고전적 작용량을 $S_{\text{inst}}$라 할 때, 터널링 확률 밀도는 기하학적으로 완벽히 결정된다:
$$\gamma \propto \exp\left(-\frac{S_{\text{inst}}}{\hbar}\right)$$

우주 진화를 지배하는 핵심 물리량은 지평선을 통과하는 **엔트로피 유속(entropy flux)**이다. 단위 면적당 터널링 유속 $\Phi_0$가 일정하므로($\Phi_0 \propto \gamma$), 총 엔트로피 유입률은 지평선 면적에 비례한다:
...
물리적 시간은 근본 좌표가 아니라 얽힘의 대수적 환영이다. 거시적 관측자는 액적 지평선 내부에 갇혀 있으므로, 관측 가능한 물리 상태는 비가환 폰 노이만 대수 $\mathcal{M}$로 기술된다.
